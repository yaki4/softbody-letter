...
void main() {
    vec2 uv = v_uv;
    vec3 map = texture2D(u_texture, uv).rgb;
    vec3 color = map;
    ...
    gl_FragColor = vec4(mix(u_bgColor, color, (1.0 - 0.65 * u_blurFactor)) + hash13(vec3(gl_FragCoord.xy, u_ditherSeed)) / 255.0, 1.0);

    gl_FragColor.rgb = mix(map, gl_FragColor.rgb, u_opacity);
}
