...
uniform vec3 u_lightFieldVolumeOffset; // this volume offset include the half grid padding on each sides
uniform vec3 u_lightFieldVolumeSize; // this volume size include the half grid padding on each sides

vec2 lightFieldGridToUv (vec3 grid) { // assume the grid is clamped to within the 0.5 - (u_lightFieldGridCount - 0.5) range
    vec2 uv = grid.xy;
    vec2 colRow = floor(vec2(
      mod(grid.z, u_lightFieldSliceColRowCount.x),
      grid.z / u_lightFieldSliceColRowCount.x
    ));
