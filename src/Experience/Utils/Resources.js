                ...
                fetch(source.path)
                    ...
                    .then(buffer =>
                    {
                        ...
                        let t = new Uint32Array(e, 0, 1)[0],
                            i = JSON.parse(String.fromCharCode.apply(null, new Uint8Array(e, 4, t))), n = i.vertexCount,
                            r = i.indexCount, a = 4 + t, l = new THREE.BufferGeometry, u = i.attributes, c = !1, f = {};
                        for (let v = 0, S = u.length; v < S; v++) {
                            let g = u[v], x = g.id, M = x === "indices" ? r : n, A = g.componentSize, b = window[g.storageType],
                                w = new b(e, a, M * A), C = b.BYTES_PER_ELEMENT, P;
                            if (g.needsPack) {
                                let E = g.packedComponents, T = E.length, L = g.storageType.indexOf("Int") === 0, F = 1 << C * 8,
                                    k = L ? F * .5 : 0, q = 1 / F;
                        ...
                        let p = i.meshType, _ = [];
                        if (i.sceneData) {
                            let v = i.sceneData, S = new THREE.Object3D, g = [], x = p === "Mesh" ? 3 : p === "LineSegments" ? 2 : 1;
